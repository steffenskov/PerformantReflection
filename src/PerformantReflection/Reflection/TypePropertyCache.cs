...
namespace PerformantReflection.Reflection
{
    ...
    {
        private static readonly LockedConcurrentDictionary<Type, IReadOnlyCollection<PropertyData>> _typeProperties = new();
        ...
        private static IReadOnlyCollection<PropertyData> CreatePropertyDatas(Type type)
        {
            ...
            {
                result.Add(new PropertyData(property.Name, true, property.Type, property.Getter, property.Setter));
            }
            foreach (var property in GetPropertyMethods(type, false))
            {
                result.Add(new PropertyData(property.Name, false, property.Type, property.Getter, property.Setter));
            }
